...
template <bool preApply, bool postApply, typename F /* a <: AstNode, b <: a => const a& -> unique_ptr<b> */>
struct LambdaMapper : public AstNodeMapper {
    ...
    using B = typename lambda_traits<F>::result_type;

    static bool apply(AstProgram& program, F&& f) {
        LambdaMapper<preApply, postApply, F> mapper{std::forward<F>(f)};
    ...
    LambdaMapper(F&& fn) : fn(std::forward<F>(fn)) {}

    std::unique_ptr<AstNode> operator()(std::unique_ptr<AstNode> node) const override {
        if constexpr (preApply) node->apply(*this);
        ...
        if constexpr (postApply) node->apply(*this);
...
template <typename F>
bool mapTopDown(AstProgram& program, F&& f) {
    return LambdaMapper<false, true, F>::apply(program, std::forward<F>(f));
}

template <typename F>
bool mapBottomUp(AstProgram& program, F&& f) {
    return LambdaMapper<true, false, F>::apply(program, std::forward<F>(f));
}
...
bool SumInitToRecordInitTransformer::transform(AstTranslationUnit& tu) {
    ...
    return mapBottomUp(*tu.getProgram(), [&](const AstSumInit& r) -> std::unique_ptr<AstRecordInit> {
        auto& ty = dynamic_cast<const SumType&>(env.getType(r.type));
        assert(ty.getBranches().size() <= MAX_RAM_UNSIGNED);

        RamDomain branchId = 0;
        for (auto&& br : ty.getBranches()) {
            if (br.name == r.getBranch()) {
                auto c = std::make_unique<AstRecordInit>(r.type);
                c->addArgument(std::make_unique<AstNumericConstant>(
                        toString(branchId), AstNumericConstant::Type::Uint));
                c->addArgument(clone(r.argument));
                return c;
            }
